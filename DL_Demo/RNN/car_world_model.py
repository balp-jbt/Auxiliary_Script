...
class WorldModel(nn.Module):
    def __init__(self, action_size, hidden_size, output_size):
        ...
        # LSTM layer
        self.lstm = nn.LSTM(cnn_output_size + action_size, hidden_size, batch_first=True)

        # Fully connected layer to predict the next state
    ...
    def forward(self, state, action, hidden=None):
        ...
        # Process the image state through the CNN
        batch_size = state.size(0)
        features = self.cnn(state)  # [batch_size, cnn_output_size]

        # Concatenate the CNN features with the action vector
        ...
        # Predict the next state using the fully connected layer
        next_state_pred = self.fc(lstm_output.squeeze(1))  # [batch_size, output_size]
